...
const login = async (req, res) => {
  try {
    ...
    if (!user) {
      console.error("Incorrect credentials: user not found");
    ...
    if (!isMatch) {
      console.error("Incorrect credentials: password mismatch");
      return res.status(400).send("Incorrect credentials");
    }
...
const updateprofile = async (req, res) => {
  try {
    ...
    console.log("Updating user profile with ID:", req.user.userId);
